...
if ~isfield(input,'nanString'),input.dataNanString = '\miss';end
...
cellSize = [numberDataRows+rowLabelsExist,numberDataCols+colLabelsExist];
C = cell(cellSize);
...
for i=1:size(C,1)
    ...
    for j=1:size(C,2)
        ...
        elseif isnumeric(dataValue)
            ...
            if (i>1 && j>1)
                dataValue = [num2str(dataValue,dataFormatArray{i,j}),' & (',num2str(err(i-1,j-1)),')'];
            else
                dataValue = num2str(dataValue,dataFormatArray{i,j});
            end
        end
